...
case "$HOME" in
  *termux*) echo "autopif2: need su root environment"; 
#  exit 1;;
esac;
...
find_busybox() {
  [ -n "$BUSYBOX" ] && return 0;
  local path;
  for path in $(which busybox) /data/adb/modules/busybox-ndk/system/*/busybox /data/adb/magisk/busybox /data/adb/ksu/bin/busybox /data/adb/ap/bin/busybox; do
    if [ -f "$path" ]; then
      BUSYBOX="$path";
      return 0;
    fi;
...
echo "BETA_REL_DATE: $BETA_REL_DATE";
echo "BETA_EXP_DATE: $BETA_EXP_DATE";
...
for MIGRATE in ./old_migrate.sh migrate.sh /data/adb/modules/playintegrityfix/migrate.sh; do
  [ -f "$MIGRATE" ] && break; 
done;
...
if [ -f "$MIGRATE" ]; then
  ...
  item "Converting pif.json to custom.pif.json with $MIGRATE:";
