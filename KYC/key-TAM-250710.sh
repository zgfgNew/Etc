...
# Modified from Tricky Addon Module v3.9 (KOWX712)
# Adapted by osm0sis@xda

echo -e "\n=== TAM Keybox Retriever ===";

case "$0" in
  *.sh) DIR="$0";;
  *) DIR="$(lsof -p $$ 2>/dev/null | grep -o '/.*key-TAM-.*.sh$')";;
esac;
...
URLS="
# AOSP Keybox URL
#https://raw.githubusercontent.com/KOWX712/Tricky-Addon-Update-Target-List/main/module/common/.default
...
for URL in $(echo "$URLS" | grep -v '^#'); do
  ...
  unset KEY_CONTENT;
  KEY_CONTENT="$(wget -q -O - --no-check-certificate "$URL" | xxd -r -p | base64 -d)";
  [ "$DUMPRAW" ] && echo "$KEY_CONTENT" > $USER-$(basename "$URL").raw
  [ ! "$KEY_CONTENT" ] && skip "Empty file" && continue;
